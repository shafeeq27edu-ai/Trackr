...
@router.post("")
def create_stream(request: StreamCreateRequest, stream_manager: StreamManager = Depends(get_stream_manager), current_user: User = Depends(get_current_user)):
    stream = stream_manager.create_stream(request.source)
    return {"id": stream.id, "source": stream.source, "status": stream.status}

@router.get("")
def list_streams(stream_manager: StreamManager = Depends(get_stream_manager), current_user: User = Depends(get_current_user)):
    streams = stream_manager.list_streams()
    return {"streams": [s.model_dump() for s in streams.values()]}

@router.get("/{stream_id}")
def get_stream(stream_id: str, stream_manager: StreamManager = Depends(get_stream_manager), current_user: User = Depends(get_current_user)):
...
def stop_stream(stream_id: str, stream_manager: StreamManager = Depends(get_stream_manager), current_user: User = Depends(get_current_user)):
    stream = stream_manager.get_stream(stream_id)
    if not stream:
        raise HTTPException(status_code=404, detail="Stream not found")
...
def delete_stream(stream_id: str, stream_manager: StreamManager = Depends(get_stream_manager), current_user: User = Depends(get_current_user)):
...
def record_stream(stream_id: str, stream_manager: StreamManager = Depends(get_stream_manager), current_user: User = Depends(get_current_user)):
    ...
    recording_path = f"outputs/live_recordings/recording_{stream_id}_{int(asyncio.get_event_loop().time())}.mp4"
...
async def websocket_global_status(websocket: WebSocket, stream_manager: StreamManager = Depends(get_stream_manager)):
