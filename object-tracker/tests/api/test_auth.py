...
def test_register_password_too_short(auth_client):
    res = auth_client.post(
        "/api/v1/auth/register",
        json={"email": "shortpass@example.com", "password": "short", "username": "shortpass"},
    )
...
def test_register_invalid_email(auth_client):
    res = auth_client.post(
        "/api/v1/auth/register",
        json={"email": "not-an-email", "password": "Password123!", "username": "notanemail"},
    )
    assert res.status_code == 422
...
def test_login_wrong_password(auth_client, test_user):
    ...
    res_nonexistent = auth_client.post(
        "/api/v1/auth/login",
        data={"username": "doesnotexist@example.com", "password": "SomePassword1!"},
    )
