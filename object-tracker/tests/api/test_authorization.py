...
def users(auth_client):
    ...
    tokenA = auth_client.post(
        "/api/v1/auth/login", data={"username": userA["email"], "password": userA["password"]}
    ).json()["access_token"]

    # User B
    userB = {"email": "userb@example.com", "password": "Password123!", "username": "userb"}
    auth_client.post("/api/v1/auth/register", json=userB)
    tokenB = auth_client.post(
        "/api/v1/auth/login", data={"username": userB["email"], "password": userB["password"]}
    ).json()["access_token"]
...
def test_idor_project_access(auth_client, users):
    ...
    res_b = auth_client.delete(
        f"/api/v1/projects/{project_id}", headers={"Authorization": f"Bearer {tokenB}"}
    )
    assert res_b.status_code in [403, 404]

    # User A CAN delete their own project
    res_a = auth_client.delete(
        f"/api/v1/projects/{project_id}", headers={"Authorization": f"Bearer {tokenA}"}
    )
    assert res_a.status_code == 200
...
def test_idor_stream_access(auth_client, users):
    ...
    res_stream = auth_client.post(
        "/api/v1/streams",
        json={"source": "rtsp://test"},
        headers={"Authorization": f"Bearer {tokenA}"},
    )
    ...
    # User B tries to access it
    res_b = auth_client.get(
        f"/api/v1/streams/{stream_id}", headers={"Authorization": f"Bearer {tokenB}"}
    )
    assert res_b.status_code in [403, 404]

    # User A CAN access their own stream
    res_a = auth_client.get(
        f"/api/v1/streams/{stream_id}", headers={"Authorization": f"Bearer {tokenA}"}
    )
    assert res_a.status_code == 200
