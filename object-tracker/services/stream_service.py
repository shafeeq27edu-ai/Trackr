...
async def process_live_stream(
    stream_id: str, source: str, stream_manager: StreamManager, detector: YoloDetector
):
    """
    ...
    if not reader.is_opened:
        ...
        stream_manager.update_stream(
            ...
            error="Unable to open webcam. Camera may be in use.",
        ...
        while True:
            ...
            if not ret or frame is None:
                ...
                if not reader.is_opened:
                    stream_manager.update_stream(
                        stream_id,
                        status=StreamStatus.FAILED,
                        error="Unable to reconnect to camera.",
                    )
                    break
                stream_manager.update_stream(
                    stream_id, status=StreamStatus.PLAYING, error=None, camera_connected=True
                )
                continue
            ...
            if frames_processed % 15 == 0:
                stream_manager.update_stream(
                    ...
                    total_detections=(
                        analytics.total_detections if hasattr(analytics, "total_detections") else 0
                    ),
                )
