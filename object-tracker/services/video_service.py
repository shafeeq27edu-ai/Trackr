...
async def process_video_file(
    ...
    try:
        ...
        with sv.VideoSink(
            target_path=output_path, video_info=output_video_info, codec=codec
        ) as sink:
            frame_idx = 0
            batch_size = 1
            frame_batch = []
            ...
            def process_batch():
                ...
                for b_frame, b_detections in zip(frame_batch, batch_detections):
                    ...
                    has_tracks = (
                        b_detections.tracker_id is not None and len(b_detections.tracker_id) > 0
                    )
                    ...
                    if has_tracks:
                        ...
                        for class_id, confidence, tracker_id in zip(
                            ...
                            try:
                                class_name = (
                                    detector.names[int(class_id)]
                                    if getattr(detector, "names", None)
                        ...
                        annotated_frame = trace_annotator.annotate(
                            scene=annotated_frame, detections=b_detections
                        )
        ...
        if codec != "avc1":
            ...
            await job_manager.update_job(
                job_id, status=JobStatus.PROCESSING, stage="Finalizing Video (Encoding)"
            )
        ...
        await job_manager.update_job(job_id, status=JobStatus.FAILED, error=str(e), stage="Failed")
