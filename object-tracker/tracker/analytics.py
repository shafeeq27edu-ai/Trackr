...
class AnalyticsEnginePlugin(BaseAnalytics):
    ...
    def __init__(self, log_dir: str = "outputs", fps: float = 30.0):
        ...
        self.dwell_times: Dict[int, Dict[str, int]] = (
            {}
        )  # tracker_id -> {'first': frame, 'last': frame}
        self.tracker_classes: Dict[int, str] = {}  # tracker_id -> class_name
        self.zone_states: Dict[str, Dict[int, bool]] = (
            {}
        )  # zone_id -> {tracker_id -> bool (is_in_zone)}
    ...
    def process_zone_triggers(
        self, zone_id: str, trigger_results: List[bool], tracker_ids: List[int]
    ):
    ...
    def process_detections(
        self, detections: sv.Detections, class_names: Dict[int, str], frame_idx: int
    ) -> None:
    ...
    def generate_session_summary(self, total_frames: int, duration_sec: float) -> Dict:
        ...
        class_distribution = {
            class_name: len(ids) for class_name, ids in self.unique_counts.items()
        }
        ...
        summary = {
            "video_stats": {
                ...
                "avg_processing_fps": (
                    round(total_frames / duration_sec, 2) if duration_sec > 0 else 0
                ),
            },
            "traffic_stats": {
                ...
                "average_objects_per_frame": (
                    round(self.total_detections / total_frames, 2) if total_frames > 0 else 0
                ),
            },
            "class_distribution": class_distribution,
            "dwell_times_sec": {"average": round(avg_dwell, 2), "maximum": round(max_dwell, 2)},
            "speed_stats": {
